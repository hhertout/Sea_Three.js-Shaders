...
void main() {
    ...
    float elevationX = sin(modelPosition.x * uBigWavesFrequency.x + uTime * uBigWaveSpeed);
    float elevationZ = sin(modelPosition.z * uBigWavesFrequency.y + uTime * uBigWaveSpeed);
    float elevation =  elevationX * elevationZ * uBigWavesElevation;

    for(float i = 1.0; i < uSmallWaveIteration; i++){
        float cnoiseElevation = cnoise(
        vec3(
            modelPosition.xz * uSmallWaveFrequency * i ,
            uTime * uSmallWaveSpeed
            )
        ) * uSmallWaveElevation / i;
        elevation -= abs(cnoiseElevation);
    }
